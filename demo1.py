...
def setBtnGraph(num):
    ...
    if num == 0:
        if flag:
            ...
            btn0.config(text="X")   
        btn0.config(state=DISABLED)     #state=DISABLED(常數):讓已經選取過的按鈕不會再變動 ps:也可以改成"disable" 
    elif num == 1:
        if flag:
            btn1.config(text="O")
        else:
    ...
    elif btn0.cget("text") == btn3.cget("text") and btn0.cget("text") == btn6.cget("text"):
        if btn0.cget("text") == "O":
            print("Player1 is Winner")
        elif btn0.cget("text") == "X":
            print("Player2 is Winner")
    ...
    elif btn0.cget("text") == btn4.cget("text") and btn0.cget("text") == btn8.cget("text"):
        if btn0.cget("text") == "O":
            print("Player1 is Winner")
        elif btn0.cget("text") == "X":
            print("Player2 is Winner")
...
btn0 = Button(window, text="", command=lambda:setBtnGraph(0))       #lambda:導入的函數要帶有參數時
btn0.grid(row=0, column=0, sticky=NSEW)                             #sticky:放大，NSEW:東西南北
btn1 = Button(window, text="", command=lambda:setBtnGraph(1))
btn1.grid(row=0, column=1, sticky=NSEW)
btn2 = Button(window, text="", command=lambda:setBtnGraph(2))
btn2.grid(row=0, column=2, sticky=NSEW)

btn3 = Button(window, text="", command=lambda:setBtnGraph(3))
btn3.grid(row=1, column=0, sticky=NSEW)         
btn4 = Button(window, text="", command=lambda:setBtnGraph(4))
btn4.grid(row=1, column=1, sticky=NSEW)
btn5 = Button(window, text="", command=lambda:setBtnGraph(5))
btn5.grid(row=1, column=2, sticky=NSEW)

btn6 = Button(window, text="", command=lambda:setBtnGraph(6))
btn6.grid(row=2, column=0, sticky=NSEW)         
btn7 = Button(window, text="", command=lambda:setBtnGraph(7))
btn7.grid(row=2, column=1, sticky=NSEW)
btn8 = Button(window, text="", command=lambda:setBtnGraph(8))
